...
@Component({
	...
	imports: [EntityError, EntityNameInput, ButtonControl],
...
export class ProjectEdit {
	...
	protected async _rename() {
		...
		if (project !== null) {
			const result = await this._service.rename(project.id, this._newName());

			if (!result.success) {
				return this._messageBox.messageConfig.set({ type: 'error', text: this._getRenameErrorText(result.code) });
			}

			this._project.set({ ...project, name: result.name });
			this._newName.set('');
		}
	}
}
